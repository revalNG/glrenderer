...
type
  ...
  public
    class function Init(aRenderer: IdfRenderer; aWorld: Tdfb2World; aParams: TglrCarParams): TglrCar;
...
class function TglrCar.Init(aRenderer: IdfRenderer; aWorld: Tdfb2World; aParams: TglrCarParams): TglrCar;
var
  finalSize: TdfVec2f;
...
begin
  ...
  begin
    FSprite := dfNewSpriteWithNode(R.RootNode);
    dfLoadSprite(FSprite, spriteFileName, position, rotation);
    finalSize := dfVec2f(FSprite.Width, FSprite.Height) + bodySizeOffset;

    //Тело машины
    FBody := dfb2InitBox(aWorld, position, finalSize, rotation, density, friction, restitution, $0002, $0004);
